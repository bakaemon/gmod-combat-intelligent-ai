...
BR.RegisterHook("brain/react", "all_grenade_dodge", function(data, dt)
    local npc = data.ent
    local now = CurTime()
    ...
    if biasVec then data.reflex.bias = (data.reflex.bias or Vector(0,0,0)) + biasVec; data.reflex.urgency = urgency end
end)
