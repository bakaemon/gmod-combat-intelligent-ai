...
local Ranged = Engage.NewCore()
BR.Exec.Engage.Ranged = Ranged

function Ranged.Run(data)
    local npc = data.ent
    ...
    if decision == "point_blank_fight" then
        ...
        if data.pbPhase == "fire" and dist < pcfg.WithdrawDist then
            ...
            away:Normalize()
            local dest = CAI.Nav.SafeOffset(npc:GetPos(), CAI.SpatialMap.QuerySafeDir(data.squad, npc:GetPos(), away), pcfg.Step)
    ...
    if resp == CAI.Config.WeaponResponses.melee then
        if dist < ideal * 0.95 then
            if now - (data.kiteAt or 0) > 1.2 then
                ...
                local dest = CAI.Nav.SafeOffset(npc:GetPos(), CAI.SpatialMap.QuerySafeDir(data.squad, npc:GetPos(), away), 260)
                if dest then CAI.Nav.MoveTo(data, dest, "run") end
            end
        end
        CAI.FriendlyFire.Update(data)
    ...
    if dist < CAI.Config.Engage.PointBlank * 2 and CAI.Util.Sees(npc, enemy)
        ...
        local away = npc:GetPos() - enemy:GetPos()
        away.z = 0 away:Normalize()
        local dest = CAI.Nav.SafeOffset(npc:GetPos(), CAI.SpatialMap.QuerySafeDir(data.squad, npc:GetPos(), away), 200)
        if dest then CAI.Nav.MoveTo(data, dest, "run") end
    end
    ...
    if dist < CAI.Config.Engage.PointBlank and not CAI.Util.Sees(npc, enemy) then
        ...
        if Ranged.overrides.push then
            local handled, stop = Ranged.Dispatch("push", {
                data = data, npc = npc, enemy = enemy, dist = dist,
                ideal = ideal, maxRange = maxRange, now = now,
                tryMoveShoot = tryMoveShoot, safeDest = safeDest,
            })
